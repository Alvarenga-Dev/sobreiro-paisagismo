...
import { catalogProjects, projectCategories, projectContactActions, projectsContent } from "./projectsContent";
...
describe("Projetos", () => {
  ...
  it.each(["paisagismo-comercial", "todos"] as const)("oferece recuperação com coleção vazia em %s", (category) => {
    render(<><ProjectsCatalog projects={[]} category={category} destinations={destinations} /><ContactBanner id="contact" {...projectsContent.banner} {...projectContactActions({ status: "unavailable" }, siteContent.contact)} /></>);
  ...
  it("banner mantém API anterior e usa WhatsApp somente quando configurado", () => {
    const { rerender } = render(<ContactBanner id="contact" message={projectsContent.banner.message} {...projectContactActions({ status: "unavailable" }, siteContent.contact)} />);
    expect(screen.queryByText(projectsContent.banner.description)).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "Fale no WhatsApp" })).not.toBeInTheDocument();
    rerender(<ContactBanner id="contact" {...projectsContent.banner} {...projectContactActions({ status: "configured", href: "https://example.com/whatsapp" }, siteContent.contact)} />);
